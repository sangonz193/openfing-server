...
import { createLegacyJsonFilesConfig } from "../../config/createLegacyJsonFiles.config"
...
export async function tryCreateLegacyJsonFiles(
	...
	const courses = await courseRepository.findAll({ includeDisabled: true, includeHidden: true })
	...
	await arrayMapLimit(
		async (index) => {
			...
			const courseClassEditions = await courseEditionRepository.findAll({
			...
			await arrayMapLimit(
				async (index) => {
					const value = courseClassEditions[index]
					const courseEditionMap = await handleCourseEdition(value, {
						...options,
						course,
					})
	...
	await Promise.all([
		...
		arrayMapLimit(
			async (index) => {
				const detailedCourse = detailLegacyCourses[index]
				await compareAndWriteFile(
					path.resolve(coursesJsonFilePath, "..", `${detailedCourse.code}.json`),
					detailedCourse
				)
			},
			detailLegacyCourses.length,
			2
		),
	])
...
async function handleCourseClassList(
...
async function handleCourseEdition(
	...
	return arrayMapLimit<{ course: LegacyCourse | undefined; courseDetail: LegacyCourseDetail }>(
		async (index) =>
			handleCourseClassList(courseClassLists[index], {
				course,
